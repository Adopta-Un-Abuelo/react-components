...
const ChipsContainerSmall = styled.div<{ disabled?: boolean }>`
	...
	background-color: ${(props) =>
		props.disabled ? Color.background.soft : Color.background.primaryLow};
`;
...
const ChipSelector = styled.div<{ disabled?: boolean }>`
	...
	height: 32px;
	border: 1px solid
		${(props) => (props.disabled ? Color.line.low : Color.line.full)};
...
const Label = (props: Props) => {
	...
	const [selectedColor, setSelectedColor] = useState<
		{ color: string; backgroundColor: string; text: string } | undefined
	>(undefined);
	...
	return props.type === "chip" ? (
		props.size === "big" ? (
			<ChipsContainerBig role="chip" {...restProps}>
				<Text
					type="p2"
					weight="medium"
					style={{
				...
				>
					{props.text.slice(0, 1).toLocaleUpperCase() +
						props.text
							.slice(1, props.text.length)
							.toLocaleLowerCase()}
				</Text>
			</ChipsContainerBig>
		) : props.size === "selector" ? (
			<ChipSelector role="chip" {...restProps}>
