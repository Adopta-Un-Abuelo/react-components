...
const Box = styled.div<{selected: boolean}>`
    ...
    border: ${props => props.selected ? '1px solid '+ Color.background.primary : '1px solid '+ Color.line.primarySoft};
    border-radius: 4px;
`
    ...
    useEffect(() =>{
        if(props.selectedOptions){
            ...
            props.options.map(item => {
                if(props.selectedOptions && props.selectedOptions.findIndex(temp => temp.id === item.id) > -1){
                    temp.push(item);
                }
            });
            setSelected(temp);
                    ...
                    {props.options.map((item)=>{
                        ...
                        return (
                            ...
                            >
                                ...
                                {item.icon}
                                <Text type='p' style={{color:`${Color.text.full}`}}>
                                    {item.title}
                                </Text>
                            </Option>
                        )
