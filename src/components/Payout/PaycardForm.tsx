import { useState, useEffect, forwardRef, useImperativeHandle, Ref, CSSProperties } from 'react';
...
const PayoutForm = forwardRef((props: FormProps, ref: Ref<FormRef>) =>{
    ...
    const [ inputEmailError, setInputEmailError ] = useState<string | undefined>(undefined);
    const [ inputPaymentError, setInputPaymentError ] = useState<string | undefined>(undefined);
    ...
    const createPaymentMethod = async (): Promise<PaymentMethod | undefined> => {
        if(elements && stripe && paymentMethod && name){
            ...
            if(!name)
                setInputNameError('Debes añadir el nombre del titular');
            if(!paymentMethod)
